...
const animationClass = computed(() => props.animated ? 'skeleton-pulse-slow' : '');
...
<template>
  ...
  <div v-else-if="type === 'card'" class="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    ...
    >
      ...
      <div class="flex gap-2 justify-end">
        <div
          class="h-8 w-20 rounded-[var(--radius-control)] skeleton-gradient"
        ></div>
        <div
          class="h-8 w-16 rounded-[var(--radius-control)] skeleton-gradient"
        ></div>
      </div>
    </div>
  </div>
  ...
  <div v-else-if="type === 'file-card'" class="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-4">
    ...
    >
      ...
      >
        <div class="h-28 relative overflow-hidden rounded-t-[var(--radius-card)]">
          <div
  ...
  <div v-else-if="type === 'form'" class="space-y-4">
    <div v-for="i in 5" :key="i" class="space-y-2">
      ...
      <div
        class="h-10 w-full rounded-[var(--radius-control)] skeleton-inline"
        :class="[animationClass, shimmerClass]"
      ></div>
    </div>
    ...
    <div class="flex gap-3 pt-6">
      ...
      ></div>
      <div
        class="h-10 w-20 rounded-[var(--radius-control)] skeleton-gradient"
        :class="[animationClass, shimmerClass]"
      ></div>
    </div>
  </div>
        ...
        <div v-for="i in 4" :key="i" class="space-y-2">
          <div class="menu-item flex items-center gap-2 px-3 py-2 rounded-[var(--radius-control)]">
            ...
            <div class="flex items-center gap-1 ml-auto">
              <div class="h-5 w-14 bg-[var(--surface-muted)] rounded-full skeleton-gradient" :class="[animationClass, shimmerClass]"></div>
              <div class="h-5 w-16 bg-[var(--surface-muted)] rounded-full skeleton-gradient" :class="[animationClass, shimmerClass]"></div>
              <div v-if="i % 2 === 0" class="h-5 w-20 bg-[var(--surface-muted)] rounded-full skeleton-gradient" :class="[animationClass, shimmerClass]"></div>
            </div>
          </div>
          <div v-if="i <= 2" class="pl-4 md:pl-6 space-y-1">
            <div class="menu-item-child flex items-center gap-2 px-3 py-1.5 rounded-[var(--radius-control)]">
              ...
              <div class="flex items-center gap-1 ml-auto">
                <div class="h-4 w-12 bg-[var(--surface-muted)] rounded-full skeleton-gradient" :class="[animationClass, shimmerClass]"></div>
              </div>
            </div>
          </div>
        </div>
