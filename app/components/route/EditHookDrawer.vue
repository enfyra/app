<template>
  <CommonDrawer
    ...
    :danger-action="{ label: 'Delete', onClick: () => emit('delete') }"
    :primary-action="{
      label: 'Save Hook',
      loading,
  ...
  >
    <template #header>
      <h2 class="text-xl font-semibold">Edit {{ hookType === 'pre' ? 'Pre' : 'Post' }}-Hook</h2>
    </template>
    <template #body>
      <div class="space-y-6">
...
const fieldMap = computed(() => ({
  ...
  sourceCode: getRouteHookSourceCodeFieldConfig({
    ...
    hookId: localForm.value?.id ?? localForm.value?._id,
