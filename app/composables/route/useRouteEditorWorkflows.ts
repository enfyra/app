import { validateRouteHandlerForm, validateRouteHookForm } from "~/utils/route-handler-hook-forms";

type MethodObject = { name: string; id?: string };

type RouteEditorWorkflowOptions = {
  routeId: Ref<string | undefined>;
  idField: string;
  availableMethodStrings: ComputedRef<string[]>;
...
export function useRouteEditorWorkflows(options: RouteEditorWorkflowOptions) {
  ...
  const handlerAvailableMethods = computed(() =>
    options.availableMethodStrings.value.filter((m: string) => !handlerOccupiedMethods.value.has(m)),
  );
  ...
  const defaultHandler = computed(() => {
    if (!options.mainTableName.value || handlers.value.length > 0) return null;
    return {
      _isDefault: true,
      name: "Built-in logic",
      description: `Built-in CRUD logic for ${options.mainTableName.value}.`,
    };
  });
  ...
  async function refreshRouteWorkflows() {
    await Promise.all([
      fetchHandlers(),
      fetchPreHooks(),
      fetchPostHooks(),
  ...
  function resolveMethodObject(methodStr: string): MethodObject {
  ...
  watch(editHandlerData, (data) => {
    if (data?.data?.[0]) {
      ...
      editHandlerForm.value.route = { [options.idField]: options.routeId.value };
    }
  });
  ...
  async function editHook(hook: any) {
    ...
    if (type === "pre") {
      ...
      if (editPreHookData.value?.data?.[0]) {
        ...
        editHookForm.value.route = { [options.idField]: options.routeId.value };
  ...
  return {
    ...
    preHooksLoading,
    postHooksLoading,
    globalPreHooksLoading,
    globalPostHooksLoading,
    handlers,
