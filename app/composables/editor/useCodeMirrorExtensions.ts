...
export function useCodeMirrorExtensions(codeMirrorModules?: Ref<any> | any) {
  ...
  function buildEnfyraDecorations(view: any): any {
    ...
    for (let i = 1; i <= doc.lines; i++) {
      ...
      const templateRegex = /@(CACHE|REPOS|HELPERS|STORAGE|LOGS|ERRORS|BODY|ENV|DATA|STATUS|PARAMS|QUERY|USER|REQ|RES|SHARE|API|UPLOADED_FILE|PKGS|SOCKET|DISPATCH|TRANSACTION|FLOW|FLOW_LAST|FLOW_META|FLOW_PAYLOAD)\b/g
      const templateWithBracketRegex = /@(CACHE|REPOS|HELPERS|STORAGE|LOGS|ERRORS|BODY|ENV|DATA|STATUS|PARAMS|QUERY|USER|REQ|RES|SHARE|API|UPLOADED_FILE|PKGS|SOCKET|DISPATCH|TRANSACTION|FLOW|FLOW_LAST|FLOW_META|FLOW_PAYLOAD)\s*\[(['"])([^'"]*)\1\]/g
